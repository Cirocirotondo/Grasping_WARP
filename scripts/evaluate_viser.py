...
Isaac Lab runs headless in this process; Viser renders what it reports. The
...
``uv pip install --python deps/IsaacLab/.venv/bin/python yourdfpy``):
    ...
    env -u PYTHONPATH deps/IsaacLab/.venv/bin/python scripts/evaluate_viser.py \
...
from evaluate import load_saved_configuration, termination_reason  # noqa: E402
from simtoolreal_newton.cfg import SimToolRealCfg  # noqa: E402
from simtoolreal_newton.envs.transform_bank import (  # noqa: E402
...
from simtoolreal_newton.runners.eval_plotter import EvaluationPlotter  # noqa: E402
...
# The URDF's mesh filenames are written relative to assets/, not to the file.
URDF_MESH_ROOT = REPO_ROOT / "assets"
# The env asserts the robot base is unrotated; every quantity the env reports
# is env-local (origins subtracted), so environment 0 maps straight to Viser.
ROBOT_WXYZ_WORLD = np.asarray((1.0, 0.0, 0.0, 0.0))
...
def load_urdf_for_viser(urdf_path: Path):
    """Parse the URDF with mesh paths resolved against assets/ or the file."""
    import yourdfpy
    ...
    def resolve(fname: str) -> str:
        ...
        for base in (URDF_MESH_ROOT, urdf_path.parent):
            if (base / fname).is_file():
                return str(base / fname)
        return fname

    return yourdfpy.URDF.load(str(urdf_path), filename_handler=resolve)
...
class EvaluationViewer:
    def __init__(self, args: argparse.Namespace) -> None:
        try:
            ...
            raise RuntimeError(
                "This evaluator needs viser and yourdfpy in the project venv: "
        ...
        table_size = np.asarray(
            [float(value) for value in self.env_cfg.table.size_m]
        )
        table_center = np.asarray(
            (0.0, 0.0, self.table_top_z - table_size[2] / 2.0)
        )
        ...
        self.server.scene.add_frame(
            "/reference_robot",
            position=self.robot_position + self.ghost_offset,
            wxyz=ROBOT_WXYZ_WORLD,
            show_axes=False,
        )
    ...
    def _build_environment(self) -> None:
        ...
        self.robot_position = np.asarray(
            [float(value) for value in env_cfg.init_state.pos], dtype=np.float64
        )
    ...
    def _fingertip_heights(self):
        ...
        """
        from simtoolreal_newton.envs.keypoints import split_palm_and_fingertips
        from simtoolreal_newton.envs.motion_imitation_env import _quat_rotate
    ...
    def _rollout(
        ...
        try:
            ...
            # The same per-episode recorder scripts/evaluate.py uses, so a
            # rollout driven from the browser leaves the full eval_plots set
            # (npz + every figure) for environment 0 under eval_viser/<stem>/.
            self.plot_folder = None
            plotter = EvaluationPlotter(self.checkpoint.parent / "eval_viser")
            plotter.start_episode(self._rollout_stem(start_index), env)
            plotter_open = True
            alive = torch.ones(
            ...
            with torch.inference_mode():
                for step in range(1, horizon + 1):
                    ...
                    if plotter_open:
                        plotter.record(env, step, actions, rewards, dones, infos)
                        if bool(dones[0]):
                            self.plot_folder = plotter.finalize(
                                termination_reason(infos, 0)
                            ).get("episode_dir")
                            plotter_open = False
            ...
            if plotter_open:
                ...
                self.plot_folder = plotter.finalize(
                    "stopped" if stopped else "reference_end"
                ).get("episode_dir")
                plotter_open = False
    ...
    def _finish(
        ...
        if self.final_metrics:
            termination_note = (
                ...
                "that step is not recoverable: Isaac Lab resets a finished "
    ...
    def _build_diagnostics(self, start_index: int) -> None:
        ...
        translation, yaw_rad = self._requested_transform()
        stem = self._rollout_stem(start_index)
        ...
        note = "Saved `{}` and `{}`.".format(image_path.name, summary_path.name)
        if self.plot_folder is not None:
            note += "  \nFull evaluation figures (as `scripts/evaluate.py`): `{}/`.".format(
                Path(self.plot_folder).name
            )
        self.diagnostics_note.content = note
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--sim-device",
        default=None,
        help="Simulation device, e.g. cuda:0 (default: config).",
    )
    parser.add_argument(
        "--physics",
        default=None,
