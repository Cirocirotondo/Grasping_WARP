...
The bridge (``/home/duplo/simone/SimToolReal/deployment/simtoolreal_real/``) is
reused unmodified. It runs under the ROS 2 Humble interpreter while the policy
runs under this repository's venv; the two exchange only the latest hand state
and target as JSON over localhost UDP.
...
class HandClient:
    ...
    def poll(self) -> bool:
        ...
        while True:
            ...
            if currents is not None:
                ...
                if current_array.shape == (HAND_DOF,) and np.all(np.isfinite(current_array)):
    ...
    def require_fresh_state(self) -> np.ndarray:
        ...
        if age > self.state_timeout:
            raise HandClientError(
                "Hand state is stale by {:.3f} s (limit {:.3f} s).".format(age, self.state_timeout)
            )
    ...
    def send_target(self, positions: np.ndarray) -> None:
        ...
        self.command_socket.sendto(
            json.dumps(payload, separators=(",", ":")).encode(), self.command_endpoint
        )
