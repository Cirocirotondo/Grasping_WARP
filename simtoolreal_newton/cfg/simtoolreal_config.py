...
class SimToolRealCfg(BaseEnvCfg):
    ...
    class control:
        ...
        # deployment/run_policy_real.py, simtoolreal_newton/sim2sim/observation.py
    ...
    class rewards:
        ...
        # Which bar pose the PALM half of the keypoint term is measured from.
        # The fingertip half is always measured from the bar as measured, where
        # relative geometry is the actual subject matter.
        #
        # "measured" -- what every run up to stage1_adaptive_sigma_24k used,
        # and no longer the default -- asks: is the palm where the
        # demonstration put it *relative to the bar that is there*. During
        # Transport a firm grasp fixes that geometry, so the term goes blind to whether the bar is being lifted at all -- a policy
        # that lifts nothing scores the same as one that lifts 15 cm -- while
        # still billing whatever offset the grasp happened to end up with, an
        # error the hand cannot correct without letting go. At weight 0.80 it
        # drowns out every term asking for the lift.
        #
        # "reference" anchors the palm to the bar's *reference* pose instead,
        # so its target rises with the reference bar: a hand that has not
        # lifted accrues a growing error pointing up. Note this also changes
        # what termination.palm_keypoint_threshold_m fires on -- absolute
        # deviation from the demonstrated palm trajectory rather than relative
        # geometry -- which is deliberate: it removes "survive without lifting"
        # as a viable strategy. Absolute return is not comparable across the
        # two settings: every run before 2026-09-14 is on the other scale.
        #
        # "reference" is the default because "measured" is not a stricter or
        # looser variant of it, it is a broken one -- two runs with different
        # recipes both stalled at reference_end_fraction ~0.45 against it, and
        # tests/test_keypoint_anchor.py shows why: it scores a lift that never
        # happened identically to one that did.
        palm_keypoint_anchor = "reference"
        ...
        #
        # Off since palm_keypoint_anchor = "reference": that anchor already pins
        # the palm's absolute orientation through the three axis keypoints, so
        # the tilt is implied whenever the keypoint error is zero. Still logged
        # as palm_tilt_error_rad to check the keypoints hold it on their own.
