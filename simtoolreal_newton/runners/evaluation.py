...
from simtoolreal_newton.envs.rsi import sample_rsi_indices
        ...
        with preserve_random_state(), torch.inference_mode():
            ...
            try:
                ...
                if self.arm_action_plot_path is not None:
                    ...
                    from simtoolreal_newton.runners.eval_plotter import (
                        EvaluationPlotter,
                    )
...
class SubprocessDeterministicEvaluator:
    """Run evaluation in an isolated process beside the live training simulation."""
    ...
    def __init__(
        ...
        # Extra command-line arguments forwarded to periodic_evaluate.py, e.g.
        # the physics backend, so the evaluator simulates what training does.
    ...
    def __call__(self, iteration, runner, is_final=False):
        ...
        command = [
            ...
            self.sim_device,
            "--fixed-phases",
        ] + [str(value) for value in self.fixed_phases] + list(self.extra_arguments)
        try:
            subprocess.run(command, check=True)
            with output.open("r", encoding="utf-8") as metrics_file:
                metrics = json.load(metrics_file)
        except (subprocess.CalledProcessError, OSError, ValueError) as error:
            # This process is a second simulation on the same card as the
            ...
            if self.consecutive_failures >= 3:
                ...
                print(
                    ...
                    "iteration {}. Check GPU memory: reduce the evaluation "
                    "or training environment count.\n".format(
